...
4. O Render reconhecerá o `render.yaml` e criará:
   - o aplicativo web no plano **Free**;
   - o banco PostgreSQL no plano **Free**.
...
**Não cadastre cartão.** Se surgir a janela “Payment Information Required”, cancele e confira no resumo do Blueprint se o aplicativo e o banco aparecem como **Free**. O arquivo `render.yaml` deste pacote já fixa `plan: free` nos dois recursos.

O banco e as tabelas são preparados automaticamente na primeira inicialização.

### Limitações do plano gratuito do Render

- O aplicativo pode hibernar depois de 15 minutos sem uso e levar cerca de 1 minuto para abrir novamente.
- O PostgreSQL gratuito expira 30 dias após a criação. Antes disso, migre o banco para um serviço gratuito duradouro, como Supabase, ou atualize o plano no Render.
- O plano gratuito é adequado para teste e uso pessoal inicial, mas não oferece garantia de produção nem backups automáticos.
